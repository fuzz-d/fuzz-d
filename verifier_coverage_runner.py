...
class FuzzdRunner(Runner):
    def run(self, seed, output_dir):
        return_code = os.system(f'timeout 30 java -jar app/build/libs/app.jar verifuzz -n -s {seed} -o {output_dir}')
        return return_code

class XDSmithRunner(Runner):
    def run(self, seed, output_dir):
        return_code = os.system(f'racket xdsmith/differ-verify.rkt {seed} > {output_dir}/main.dfy')
        if return_code == 0:
            os.system(f'mv xdsmith/tmp.dfy {output_dir}/main.dfy')
        return return_code
...
def run_coverage(program, coverage_report_json, coverage_report_cobertura):
    os.system(f'./verifier_coverlet.sh {program} {coverage_report_json} {coverage_report_cobertura}')
...
if __name__ == '__main__':
    for i in range(1, 4):
        ...
        output_dir = pathlib.Path(sys.argv[2]) / "results" / f"{run_name}_verify{i}"
